...
$recaptcha_secret = '6Lf2c-0ZAAAAAOkuW5jepR1jrnWJQyZFrhSssaZm'; 
$recaptcha_response = $_POST['recaptcha_response'];
$recaptcha = file_get_contents($recaptcha_url . '?secret=' . $recaptcha_secret . '&response=' . $recaptcha_response); 
$recaptcha = json_decode($recaptcha);
// Miramos si se considera humano o robot: 
if($recaptcha->score < 0.6){
    echo '<div class="error_message">'._('Invalid Captcha').'.</div>';
	exit();
}
...
if(trim($name_contact) == '') {
	echo '<div class="error_message">'._('You must enter your Name').'.</div>';
	exit();
/*
	...
	*/
	...
	echo '<div class="error_message">'._('Please enter a valid email address').'.</div>';
	exit();
} else if(!isEmail($email_contact)) {
	echo '<div class="error_message">'._('You have enter an invalid e-mail address, try again').'.</div>';
	exit();
	} else if(trim($phone_contact) == '') {
	echo '<div class="error_message">'._('Please enter a valid phone number').'.</div>';
	exit();
} else if(!is_numeric($phone_contact)) {
	echo '<div class="error_message">'._('Phone number can only contain numbers').'.</div>';
	exit();
} else if(trim($message_contact) == '') {
	echo '<div class="error_message">'._('Please enter your message').'.</div>';
	exit();
}
...
$e_subject = 'Consulta desde web redhousehostel.com.ar';

// You can change this if you feel that you need to.
$e_body = "Mensaje de $name_contact :" . PHP_EOL . PHP_EOL;
$e_content = "\"$message_contact\"" . PHP_EOL . PHP_EOL;
$e_reply = "Contestar via email, $email_contact o al teléfono $phone_contact";
...
$user = "$email_contact";
$usersubject = _("Thank You");
$userheaders = "From: sitio@redhousehostel.com.ar\n";
...
$usermessage = _("Thank you for contact Red House Hostel. We will reply shortly!");
...
if(mail($address, $e_subject, $msg, $headers)) {
	...
	echo "<strong >"._("Email Sent")."</strong>";
	echo "<br>"._("Thank you")." <strong>$name_contact</strong>,<br> "._("Your message has been submitted. We will contact you shortly.");
	echo "</div>";
